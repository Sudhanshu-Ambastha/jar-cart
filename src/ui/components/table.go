...
func HelpTable() string {
	rows := [][]string{
		{"Command", "Description"},
		{"init", "Creates an interactive project layout or workspace scaffold with JDK locking"},
		...
		{"sync", "Synchronizes dependencies across projects or multi-module workspaces"},
		{"add <pkg>", "Adds a dependency to the project manifest"},
		{"remove <pkg>", "Removes a dependency from the project manifest"},
		{"audit", "Checks project or workspace dependencies for known vulnerabilities"},
		...
		{"build", "Builds a project or executes a workspace-wide multi-module build"},
		{"script <name>", "Executes a custom lifecycle script (supports workspace-wide execution)"},
		{"template <key> [path] | list | remove", "Registers, lists, or removes custom project layout templates"},
